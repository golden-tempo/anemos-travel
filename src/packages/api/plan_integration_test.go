...
func TestPlanSetTravelModePersistsWithItinerary(t *testing.T) {
	...
	// set_leg_gateway is the newest tail append (leg-gateway-airports
	...
	if n := len(body.Tools); n == 0 || body.Tools[n-1].Name != "set_leg_gateway" {
		t.Fatalf("tools tail = %+v, want set_leg_gateway last", body.Tools)
	}
